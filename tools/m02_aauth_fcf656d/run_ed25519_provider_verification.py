#!/usr/bin/env python3
"""Bounded outer runner for the pinned Ed25519 provider behavior check."""
...
RUNNER_RELATIVE = Path("tools/m02_aauth_fcf656d/run_ed25519_provider_verification.py")
CHILD_RELATIVE = Path("tools/m02_aauth_fcf656d/verify_ed25519_provider.py")
...
def repository_state_and_sources():
    ...
    return {"head": head, "status_porcelain": status_lines,
...
def verify_static_inputs():
    ...
    require(regular_nonsymlink(STATIC_EVIDENCE), "static evidence absent or unsafe")
    require(sha256(STATIC_EVIDENCE) == EXPECTED_STATIC_EVIDENCE_SHA256,
            "static evidence hash mismatch")
    with open(STATIC_EVIDENCE, "r", encoding="utf-8") as handle:
        baseline = json.load(handle)
    require(baseline.get("result") == "STATIC_INSTALLATION_VERIFIED",
            "static evidence result mismatch")
    ...
    baseline_inventory = baseline.get("installation_parent_inventory", [])
    expected_paths = {entry["path"] for entry in baseline_inventory}
    actual_paths = set()
    ...
    for entry in baseline_inventory:
        ...
        if entry["type"] == "dir":
            ...
            require(metadata.st_size == entry["size"] and
                    sha256(path) == entry["sha256"], "preserved file identity mismatch")
        else:
            raise RuntimeError("unexpected baseline entry type")
    return {"static_evidence_sha256": EXPECTED_STATIC_EVIDENCE_SHA256,
            "inventory_entries_verified": len(baseline_inventory),
...
def main():
    ...
    if error_text is None:
        try:
            ...
            require(parsed.get("result") in {"ED25519_PROVIDER_VERIFIED", "FAILED"},
    ...
    record = {
        "result": "ED25519_PROVIDER_POSITIVE" if positive else "FAILED",
        "error": error_text,
