...
Repository and primary-source inspection completed on 2026-08-16. The exact
upstream/fork/branch and connector state are recorded in
`knowledge/research/G26_REUSE_ASSESSMENT_2026-08-15.md`. The current AAuth
draft is `draft-hardt-oauth-aauth-protocol-10`, the same primary-source basis
used by G26; the reference connector's bundled `SPEC.md` remains an older
snapshot.

The reference connector reaches the Person Server through `/token`, not
`/permission`. Its deferred token-exchange implementation handles the Person
Server's pending interaction and polling path. No external Governance Server
consultation exists in the reference implementation. This corroborates the
resolved D-011 position that SOGA is the Person Server's governance policy; it
is not recorded as a missing hook.

Live FIDO/CTAP2 behavior is outside G26 scope under the established trust
boundary and was not investigated. B-030 requires no further G26 work.
